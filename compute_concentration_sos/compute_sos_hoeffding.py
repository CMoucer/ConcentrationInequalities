...
def polytope_hoeffding(mu, t, matrix_kappas):
    """

    :param mu: first-order moment (float)
    :param t: deviation parameter (float)
    :param matrix_kappas: decomposition of kappas (np.ndarray)
    :return:
    """
...
def concentration_sos_hoeffding_with_variance(mu_1, mu_2, t, d, n, plot_Q=False):
    """

    :param mu_1: first-order moment (float)
    :param mu_2: second-order moment (float)
    :param t: deviation parameter (float)
    :param d: dimension of the SoS (int)
    :param n: number of variables (int)
    :param plot_Q: return the approximate Q (np.ndarray)
    :return:
    """
    ...
    G = polytope_hoeffding(mu_1, t, matrix_kappas)
    ...
    problem.solve(solver=cp.MOSEK)  # , maxIteration=100, epsilonDash=1.0E-10, mpfPrecision=250)
    if plot_Q:
...
def concentration_sos_hoeffding_with_variance_simplified(mu_1, mu_2, t, d_eff, n, verbose=False):
    """
    Same code simplified with fewer variables.
    :param mu_1: first-order moment (float)
    :param mu_2: second-order moment (float)
    ...
    :return:
    """
    print(f'The effective degree will be {d_eff + 1} due to the constraints on the Hoeffding polytop')
    d = d_eff + 1
    assert n == 2, 'Code must be modified to take it into account'
    ## VARIABLES
    # Create the right variables
    monomials = np.array(monomial_vector(d, n))
    matrix_kappas, antecedent_kappas, value_kappas = kappas(monomials)
